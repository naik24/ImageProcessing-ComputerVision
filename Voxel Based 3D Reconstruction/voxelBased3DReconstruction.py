...
def readData(totalCameras = 8):
    # lists to store images, silhouettes, and projection matrices
    images = []
    silhouettes = []
    projectionMatrices = []
...
def generateVoxelGrid(silhouettes, projectionMatrices, voxelSize = 0.03):
    ...
    # building 3D voxel grid
    voxelGrid = np.zeros((len(x_range), len(y_range), len(z_range)))

    # loop to store voxels which project on all the silouetttes
    fullOutput3D = []
...
def surfaceVoxelGrid(images, projectionMatrices, voxelGrid, voxelSize = 0.03):
    ...
    # storing spatial coordinates to calculate false RGB values
    ...
    # true rgb coloring
    # creating dict to store rgb values for each surface vertex
    candidatePoints = {str(point): np.zeros((8, 3)) for point in surfaceOutput3D}
    
    # iterating through each image, computing occlusion and updating each surface vortex with the rgb of pixel in each image
    ...
    # computing per channel median rgb
    trueRGBColorVals = []
